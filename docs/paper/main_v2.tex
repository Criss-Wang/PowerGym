...
\begin{enumerate}[nosep]
    \item \textbf{Direct mode}: Synchronous \texttt{observe()} $\rightarrow$ \texttt{act()} calls. Simple, fast, suitable for centralized training.
    \item \textbf{Distributed mode}: Event-driven \texttt{step\_distributed()} with message passing. Agents react to messages and coordinate through the MessageBroker.
\end{enumerate}

This dual-mode design enables development in direct mode (fast iteration) and validation in distributed mode (realistic event-driven execution).

\paragraph{Event-driven hierarchical execution.}
The \texttt{step\_distributed()} method (Algorithm~\ref{alg:step-distributed}) is the core of \heron{}'s event-driven execution model. It is an \texttt{async} Python method that:
\begin{itemize}[nosep]
    \item Agents \textbf{react to messages} (events) from the MessageBroker rather than polling global state
    \item Enables \textbf{concurrent execution} of subordinate agents via \texttt{asyncio.gather()}
    \item Supports \textbf{heterogeneous decision rates} by allowing agents to execute independently
\end{itemize}

\begin{algorithm}[t]
\caption{\texttt{step\_distributed()} -- Event-driven hierarchical execution}
\label{alg:step-distributed}
\begin{algorithmic}[1]
\REQUIRE Agent with message\_broker, upstream\_id, subordinates
\STATE \textbf{async def} step\_distributed():
\STATE \hspace{0.5cm} \COMMENT{1. Receive from upstream via message broker}
\STATE \hspace{0.5cm} upstream\_action $\gets$ consume(action\_channel)
\STATE \hspace{0.5cm} upstream\_info $\gets$ consume(info\_channel)
\STATE \hspace{0.5cm} update\_state(upstream\_info)
\STATE
\STATE \hspace{0.5cm} \textbf{if} subordinates exist \textbf{then}
\STATE \hspace{1.0cm} \COMMENT{2. Derive and send actions to subordinates}
\STATE \hspace{1.0cm} downstream\_actions $\gets$ derive\_actions(upstream\_action)
\STATE \hspace{1.0cm} \textbf{for} sub\_id, action \textbf{in} downstream\_actions:
\STATE \hspace{1.5cm} publish(action\_channel[sub\_id], action)
\STATE
\STATE \hspace{1.0cm} \COMMENT{3. Execute subordinates concurrently}
\STATE \hspace{1.0cm} \textbf{await} asyncio.gather(*[s.step\_distributed() for s in subordinates])
\STATE
\STATE \hspace{1.0cm} \COMMENT{4. Collect info from subordinates}
\STATE \hspace{1.0cm} \textbf{for} sub\_id \textbf{in} subordinates:
\STATE \hspace{1.5cm} subordinates\_info[sub\_id] $\gets$ consume(info\_channel[sub\_id])
\STATE
\STATE \hspace{0.5cm} \COMMENT{5. Execute local action and publish state}
\STATE \hspace{0.5cm} local\_action $\gets$ derive\_local\_action(upstream\_action)
\STATE \hspace{0.5cm} execute(local\_action)
\STATE \hspace{0.5cm} publish\_state\_updates()
\end{algorithmic}
\end{algorithm}
...
\begin{itemize}[nosep]
    \item \textbf{Message latency}: While \texttt{step\_distributed()} supports asynchronous execution, explicit latency modeling (configurable delays, stochastic timing) is future work.
    \item \textbf{Communication realism}: We support channel-based messaging but do not yet model packet loss or bandwidth constraints.
...
\textbf{Roadmap.}
\begin{itemize}[nosep]
    \item Configurable message latency profiles (SCADA-like, V2X-like) for timing studies.
...
\begin{itemize}[nosep]
    \item A three-level agent hierarchy with \texttt{step\_distributed()} for event-driven, message-based execution.
