...
def test_simple_generator_online():
    ...
    assert ("in_service" in names) and ("out_service" in names)
    assert _get(names, vec, "in_service") == pytest.approx(1.0)
    assert _get(names, vec, "out_service") == pytest.approx(0.0)
...
def test_thermal_storage_progress_and_times():
    ...
    # 'out_service' is None => not emitted
    assert ("out_service" not in names)


def test_pv_fault_out_service_flag_and_times():
    pv = StatusBlock(
        in_service=False,
        out_service=True,
        state="fault",
    ...
    assert _get(names, vec, "in_service") == pytest.approx(0.0)
    assert _get(names, vec, "out_service") == pytest.approx(1.0)
    for k in ("t_in_state_s", "t_to_next_s", "progress_frac"):
        assert k in names
...
def test_battery_idle_minimal():
    ...
    # in_service/out_service present
    assert _get(names, vec, "in_service") == pytest.approx(1.0)
    assert _get(names, vec, "out_service") == pytest.approx(0.0)
