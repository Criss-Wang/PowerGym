...
\title{Heron: A Hierarchical Multi-Agent Benchmark Framework with Composable Observability for Networked System Coordination}
...
\begin{abstract}
Real-world networked systems---from power grids and traffic networks to multi-robot warehouses and game AI---share a common challenge: agents must coordinate under organizational hierarchies, heterogeneous information access, and asynchronous communication. Yet widely used multi-agent reinforcement learning (MARL) benchmarks assume homogeneous agents, synchronous actions, and either full observability or simplistic local views, creating a critical sim-to-real gap.

We introduce \textbf{\heron{}} (Hierarchical Environments for Realistic Observability in Networks), a general-purpose MARL benchmark framework that makes these deployment-realistic constraints explicit and configurable. \heron{} provides: (1) \textbf{composable partial observability} via FeatureProviders annotated with four visibility levels (public, owner, upper\_level, system), enabling principled, role-based observability control; (2) a \textbf{three-level agent hierarchy} (Field$\rightarrow$Coordinator$\rightarrow$System) mirroring real organizational structures; (3) \textbf{dual execution modes} that separate centralized development from message-brokered distributed execution with enforced visibility filtering; and (4) a \textbf{protocol interface} that decouples communication semantics from actuation to support systematic comparisons of coordination mechanisms.

To demonstrate the framework's capabilities, we provide a complete power systems instantiation with benchmark-ready microgrid scenarios on standard IEEE/CIGRE feeders, classical baselines, and reproducible evaluation protocols. Our experiments show that the framework enables systematic study of information requirements, privacy-preserving coordination, and sim-to-real transfer gaps---research questions applicable across domains. \heron{} is released open-source with reusable abstractions designed for community adaptation to traffic, robotics, games, and other networked control domains.
\end{abstract}
...
What's needed is a \textit{benchmarking framework} that makes real deployment constraints---hierarchies, role-based observability, and asynchronous communication---first-class, so researchers can measure how coordination algorithms behave under realistic information structures and execution models.

What's needed is a \textit{general-purpose benchmarking framework} that makes these constraints first-class experimental variables, enabling researchers to systematically study how coordination algorithms behave under realistic information structures across domains. \heron{} addresses this gap by providing reusable abstractions---composable observability, hierarchical agents, protocol interfaces---that can be instantiated for any networked control domain. We demonstrate the framework through a complete power systems instantiation with standardized scenarios, metrics, and baselines.
...
\begin{itemize}[nosep]
    \item \textbf{Observability mismatch}: Policies trained with full network visibility can fail when deployed with SCADA-realistic information constraints (Section~\ref{sec:exp-mode})
...
\textbf{Heron's Approach to Addressing These Gaps.} Our framework directly confronts these limitations through domain-agnostic abstractions:
\begin{itemize}[nosep]
    \item \textbf{Hierarchical agent architecture}: Three-level system (Field$\rightarrow$Coordinator$\rightarrow$System) mirroring real organizational structures across domains with role-based capabilities
    \item \textbf{Composable partial observability}: FeatureProviders with four visibility levels (public, owner, upper\_level, system) enabling fine-grained observability control beyond binary all-or-nothing
...
These design choices enable systematic study of questions ignored by existing platforms: What is the \textit{minimum} information each agent needs for effective coordination? How much performance degrades under realistic constraints? Can hierarchical organization overcome observability limitations?

\subsection{Agent-First Philosophy}

\textbf{Why Agent-First Matters.} Traditional distributed system control is \textit{controller-centric}: a central optimizer computes commands that field entities passively execute. Even recent MARL work often treats agents as interchangeable optimizers sharing global state, ignoring the organizational reality of deployed systems---real networks are hierarchies of \textit{autonomous decision-makers} (field devices, local coordinators, system operators) with heterogeneous information access, local objectives, and communication constraints.

\textbf{Heron's Agent-First Design.} Unlike controller-centric frameworks where entities are passive actuators, \heron{} treats agents as first-class architectural elements with:
\begin{itemize}[nosep]
    \item \textbf{Heterogeneous capabilities}: FieldAgents (local actuation), CoordinatorAgents (local coordination), SystemAgent (oversight)---each with distinct roles and decision-making autonomy, not homogeneous optimizers
    ...
    \item \textbf{Organizational realism}: Agent hierarchy mirrors real organizational structures (field entities $\rightarrow$ local coordinators $\rightarrow$ system operators), enabling direct sim-to-real translation
\end{itemize}

This agent-first approach enables studying questions central to deployed multi-agent systems but ignored in existing frameworks:
\begin{itemize}[nosep]
...
We present \heron{}, a general-purpose MARL benchmark framework for networked system coordination with five key contributions:

\textbf{(1) Composable Partial Observability Framework} (\S\ref{sec:observability}): A systematic approach to controlling information access through composable FeatureProviders with embedded visibility rules. Unlike monolithic observation spaces, our framework enables fine-grained control: agents can see entity-level data (owner), aggregated subordinate data (upper\_level), system-wide metrics (system), or public information only. This maps directly to real organizational information hierarchies and enables systematic observability ablation studies across domains.

\textbf{(2) Hierarchical Agent Framework with Async Execution} (\S\ref{sec:hierarchy}, \S\ref{sec:async-execution}): A three-level agent architecture (Field$\rightarrow$Coordinator$\rightarrow$System) that mirrors real organizational structures, combined with recursive message-based execution (\texttt{step\_distributed()}) that captures asynchronous coordination patterns. This hierarchy enables scaling to large systems while maintaining computational tractability through reduced joint action space dimensionality and realistic communication patterns.

\textbf{(3) Protocol System for Coordination Strategies} (\S\ref{sec:protocols}): A composable protocol framework separating \textit{what} to communicate (CommunicationProtocol) from \textit{how} to coordinate (ActionProtocol). This enables systematic comparison of coordination mechanisms under identical observability constraints, addressing a gap in existing MARL platforms that conflate communication and control.

\textbf{(4) Dual-Mode Execution Architecture} (\S\ref{sec:dual-mode}): A unified codebase supporting centralized (full observability) and distributed (realistic) execution modes. This architecture enables algorithm development under idealized conditions while exposing observability mismatches before deployment, addressing the sim-to-real gap inherent in information-constrained systems.

\textbf{(5) Power Systems Case Study} (\S\ref{sec:benchmark}, \S\ref{sec:experiments}): A complete instantiation demonstrating the framework's capabilities through benchmark-ready microgrid scenarios on standard IEEE/CIGRE feeders, classical baselines, and reproducible evaluation protocols---serving as a template for community adaptation to other domains.

\subsection{What the Framework Enables}

\heron{} is designed to make \emph{information structure} and \emph{execution model} explicit experimental variables across networked coordination domains. The framework supports systematic studies of:
\begin{itemize}[nosep]
    \item \textbf{Minimum information for coordination}: which visibility levels (e.g., \textit{upper\_level} vs. \textit{owner-only}) are sufficient for reliable coordination in hierarchical systems.
    \item \textbf{Information--performance tradeoffs}: how performance and constraint violations change as observability is progressively restricted.
    \item \textbf{Centralized-to-distributed transfer}: how policies trained under idealized assumptions degrade when deployed under realistic filtering and message-based execution.
    \item \textbf{Privacy-preserving coordination}: how agents can cooperate through aggregate/boundary information without exposing sensitive internal states.
\end{itemize}

The underlying abstractions (FeatureProviders, hierarchical agents, protocol interface, brokered messaging) are domain-agnostic and designed for community adaptation to power systems, traffic coordination, multi-robot systems, game AI, and other networked control domains with similar organizational and information constraints.

\subsection{Platform Status and Benchmark Development Path}

We distinguish between \heron{}'s current capabilities and its potential as a cross-domain benchmark framework:

\textbf{Provided Infrastructure (v1.0):}
\begin{itemize}[nosep]
    \item reusable core abstractions (FeatureProviders, hierarchical agents, protocols, dual-mode execution)
...
This paper presents the \textit{reusable framework and demonstration in power systems}; community adaptation to other domains is the necessary next step. We provide reusable abstractions; domain experts establish domain-specific benchmarks.
...
\textbf{MARL Benchmarks and Partial Observability.}
Most MARL benchmarks (MPE \cite{mordatch2018emergence}, SMAC \cite{samvelyan2019starcraft}, PettingZoo \cite{terry2021pettingzoo}) assume homogeneous agents with either full observability or uniform observation radius. Partial observability is extensively studied in MARL theory \cite{oliehoek2016concise}, but existing benchmarks implement it through uniform observation radius or communication graph structure (CommNet \cite{sukhbaatar2016learning}). These approaches don't capture the \textit{heterogeneous information hierarchies} present in real-world systems, where different agent types have fundamentally different observability privileges based on organizational roles. Our FeatureProvider framework enables systematic study of \textit{role-based partial observability} across domains.

\textbf{Hierarchical and Communicating MARL.}
Hierarchical RL \cite{vezhnevets2017feudal} and learned communication \cite{foerster2016learning, das2019tarmac} have shown success in multi-agent coordination. Surveys of communication in MARL emphasize that performance depends on \emph{who communicates with whom}, \emph{when}, \emph{over what topology}, and \emph{with what bandwidth/latency constraints} (\todo{add citation to MARL communication survey}). Existing benchmarks rarely expose these design dimensions as controllable variables. \heron{} complements learned-communication algorithms by providing an environment substrate where communication/observability assumptions can be varied systematically via (i) visibility-labeled FeatureProviders and (ii) MessageBroker-based execution with explicit message channels.

\textbf{Domain-Specific MARL Environments.}
Various domain-specific MARL platforms exist but lack systematic observability control. In power systems: \textbf{PowerGridworld} \cite{biagioni2022powergridworld} uses monolithic observation spaces (full/local binary); \textbf{CityLearn} \cite{vazquez2019citylearn} provides building energy management without fine-grained observability; \textbf{Grid2Op} \cite{donnot2020introducing} and \textbf{gym-anm} \cite{henry2021gym} focus on single-agent control. In traffic: SUMO-based platforms lack role-based hierarchies. In robotics: multi-robot simulators typically assume full localization. None provide the composable observability framework needed for systematic study of information requirements.

\textbf{Positioning.}
Unlike domain-specific platforms that treat observability as fixed, \heron{} provides \textit{a general-purpose benchmark framework} with an agent-first design philosophy and emphasis on systematic partial observability control. We treat agents as first-class citizens with heterogeneous capabilities and role-based information access. Table~\ref{tab:comparison} compares \heron{} with existing platforms on our power systems case study; the framework's abstractions extend to traffic, robotics, and game domains (Section~\ref{sec:generalization}).

\begin{table}[t]
\centering
\caption{Comparison with existing MARL platforms (power systems case study).}
\label{tab:comparison}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Feature} & \textbf{PowerGridworld} & \textbf{CityLearn} & \textbf{gym-anm} & \textbf{\heron} \\
\midrule
...
Composable features & \xmark & \xmark & \xmark & \cmark \\
Distributed mode & \xmark & \xmark & \xmark & \cmark \\
Hierarchical agents & \xmark & \xmark & N/A & \cmark (3 levels) \\
Domain-agnostic & \xmark & \xmark & \xmark & \cmark \\
\bottomrule
\end{tabular}
\end{table}

%==============================================================================
\section{Background: Hierarchical Coordination in Networked Systems}
\label{sec:background}
%==============================================================================

Real-world networked systems---from power grids to traffic networks to multi-robot warehouses---share fundamental structural patterns that existing MARL benchmarks fail to capture. This section describes these patterns and motivates \heron{}'s design.

\subsection{The Hierarchical Coordination Problem}

Deployed multi-agent systems consistently exhibit a \textit{three-tier organizational hierarchy}:
...
This pattern appears across domains: traffic lights → intersection controllers → city traffic management; individual robots → zone coordinators → warehouse manager; combat units → squad commanders → overall commander.

\subsection{Information Constraints as First-Class Design Variables}

Real systems face \textit{information constraints} that MARL benchmarks typically ignore:
\begin{itemize}[nosep]
    ...
    \item \textbf{Regulatory/policy}: Data sharing restrictions across organizational boundaries
...
\textbf{Key Insight}: These constraints create a fundamental challenge \textit{across domains}: agents must coordinate with limited, heterogeneous, role-based information access. \heron{}'s abstractions capture this general pattern independent of physical domain.

\subsection{Case Study Domain: Power System Control}

To demonstrate the framework, we provide a complete instantiation for power system control. Modern distribution networks with 60+ controllable devices (solar inverters, batteries, EV chargers) face coordination challenges---voltage regulation, frequency response, congestion management, economic dispatch---that require multi-agent coordination under SCADA/EMS information hierarchies. This domain exemplifies the hierarchical patterns above: field devices → substation controllers → control centers.
...
The core innovation of \heron{} is a \textbf{systematic framework for controlling information access} that maps to real-world information hierarchies.
...
Creating a custom FeatureProvider (power domain example):
\begin{verbatim}
from heron.core.features import FeatureProvider
...
A key design principle of \heron{} is enabling algorithm development under idealized conditions while validating under realistic constraints.
...
Consistent with the agent-first philosophy, \heron{} implements a three-level agent hierarchy where each level represents autonomous decision-makers with distinct roles, capabilities, and information access---mirroring SCADA/EMS organizational structures.
...
\textbf{Hierarchical Reduction:} \heron{} decomposes the problem:
\begin{enumerate}[nosep]
...
\heron{} addresses this through a \textbf{recursive, message-based execution pattern} that captures realistic asynchronous coordination.
...
The asynchronous execution pattern relies on explicit message-based communication between agents. \heron{} provides a \textbf{protocol system} that separates coordination logic from agent implementation, enabling systematic study of different coordination strategies under identical observability constraints.
...
\textbf{Horizontal Protocols} (Environment-owned, Peer $\leftrightarrow$ Peer):
\begin{itemize}[nosep]
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Hybrid coordination}: Combine vertical (parent $\rightarrow$ subordinate) and horizontal (peer $\leftrightarrow$ peer) protocols
...
%==============================================================================
\section{Case Study: Power System Control}
\label{sec:experiments}
%==============================================================================

\noindent\fbox{%
\parbox{0.95\textwidth}{%
\textbf{Note on Case Study:} This section demonstrates \heron{}'s reusable abstractions through a power system control case study. While numerical values marked with \exampleval{*} are illustrative examples showing expected trends (pending full validation), the key contribution is showing how the \textit{framework's domain-agnostic abstractions enable systematic study} of observability requirements, sim-to-real gaps, and privacy-preserving coordination---capabilities that transfer to traffic, robotics, and game domains.
}%
}

\vspace{0.3cm}

To validate \heron{}'s reusable abstractions, we instantiate the framework in the power domain with 60-device microgrid networks. These experiments demonstrate that the framework enables systematic investigation of information requirements and sim-to-real gaps---research questions applicable across domains.
...
\begin{table}[t]
\centering
\caption{Observability ablation results (Summer Peak scenario, 3 microgrids). Values shown are preliminary and will be updated with the public benchmark release.}
...
\textbf{Transferable Insight}: Hierarchical coordination requires \textit{boundary observability} (upper-level features) but not full system state—a pattern applicable to traffic, robotics, and game domains.
...
\begin{table}[t]
\centering
\caption{Privacy-preserving coordination experiment. Values shown are preliminary and will be updated with the public benchmark release.}
\label{tab:privacy}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\textbf{Transferable Insight}: Agents can coordinate using aggregate/boundary information without revealing internal states—a critical requirement for multi-stakeholder systems (competing microgrids, autonomous vehicle fleets with proprietary algorithms, multi-vendor robot warehouses).
...
\begin{table}[t]
\centering
\caption{Sim-to-real transfer experiment: observability mismatch. Values shown are preliminary and will be updated with the public benchmark release.}
...
\begin{table}[t]
\centering
\caption{Scalability: flat vs. hierarchical MARL (time to reach 90\% performance). Values shown are preliminary and will be updated with the public benchmark release.}
...
\heron{}'s core abstractions---composable observability (FeatureProviders), hierarchical agents with message-brokered execution, and protocol-based coordination---are designed to be domain-agnostic. While we demonstrate the framework through power systems, the same abstractions apply to any networked system with hierarchical organization and information constraints.
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Organizational hierarchies}: Field agents → coordinators → system operators (not flat peers)
    \item \textbf{Information asymmetries}: Role-based observability, confidentiality, bandwidth limits
\end{itemize}

\textbf{Heron's Contribution}: Provide reusable abstractions for building domain-specific MARL benchmarks that capture these realities, enabling systematic study of how algorithms transfer from simulation to deployment.

\subsection{Target Application Domains}

Table~\ref{tab:domain-mapping} shows how \heron{}'s abstractions map to representative domains:

\begin{table}[t]
\centering
\caption{Cross-domain mapping of \heron{} abstractions.}
...
The recurring pattern across domains: \textit{organizational hierarchies with role-based observability}. \heron{}'s abstractions (FeatureProviders, hierarchical agents, protocols, dual-mode execution) capture this pattern independent of physical domain. Researchers can instantiate these abstractions with domain-specific feature providers and physics models to create realistic MARL benchmarks that address the "toy environment problem."
...
\textbf{Framework Philosophy}: \heron{} provides reusable abstractions (FeatureProviders, hierarchical agents, protocols, message broker). Users instantiate these abstractions with domain-specific feature providers and physics models. Below we show the power domain instantiation—other domains follow the same pattern (see §\ref{sec:generalization} for cross-domain mapping).

\textbf{Installation}:
\begin{verbatim}
pip install heron-marl
\end{verbatim}

\textbf{Configuring Observability} (Power Domain Example):
\begin{verbatim}
from heron.envs.multi_agent import MultiAgentMicrogrids
...
\textbf{Reusable Extensibility via Custom FeatureProviders}:
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Single domain instantiation}: While the framework is reusable, we only provide a full power system instantiation. Community contributions for traffic, robotics, and game domains are needed.
\end{itemize}
...
We presented \heron{} (Hierarchical Environments for Realistic Observability in Networks), a general-purpose MARL benchmark framework for networked system coordination. Unlike game-based benchmarks (MPE, SMAC) with simplified dynamics and unrealistic information structures, \heron{} provides domain-agnostic abstractions that capture organizational hierarchies, role-based partial observability, and sim-to-real transfer gaps---challenges common across power systems, traffic coordination, multi-robot systems, and game AI.

\textbf{Core Contributions}: (1) \textbf{FeatureProvider framework} for composable partial observability with fine-grained visibility rules (public, owner, upper\_level, system), enabling systematic ablation of information requirements; (2) \textbf{Hierarchical agent architecture} (3 levels: field agents → coordinators → system operators) with recursive message-based execution, reducing action space dimensionality for scalability; (3) \textbf{Protocol system} separating communication semantics from action execution, enabling heterogeneous coordination patterns; (4) \textbf{Dual-mode execution} (centralized training vs. distributed deployment) for exposing observability mismatch failures.

\textbf{Power Systems Case Study}: We demonstrate the framework through a complete microgrid instantiation with standard feeders, reference scenarios, classical baselines, and evaluation protocols. Our experiments show that \heron{}'s abstractions enable systematic study of (i) observability requirements, (ii) privacy-preserving coordination tradeoffs, and (iii) centralized-to-distributed transfer gaps.

We release \heron{} open-source to facilitate reproducible MARL research in realistic domains. The framework's domain-agnostic abstractions are designed for community adaptation to traffic, robotics, games, and other networked control domains with hierarchical organization and information constraints.
...
\begin{table}[h]
\centering
\caption{Observability ablation across all four scenarios. Values shown are preliminary and will be updated with the public benchmark release.}
\small
\begin{tabular}{@{}lccccc@{}}
\toprule
