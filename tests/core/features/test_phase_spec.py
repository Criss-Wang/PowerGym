...
def test_align_array_drops_missing_dest_phases():
    ...
    aligned = src.align_with(dst, arr, fill=-9.0)
    # C is dropped
...
def test_align_array_handles_no_overlap_gracefully():
    ...
    aligned = src.align_with(dst, arr, fill=0.0)
    np.testing.assert_allclose(aligned, np.array([0.0, 0.0], np.float32))


def test_align_with_type_and_shape_check():
    src = PhaseSpec("AB")
    dst = PhaseSpec("ABC")
    # wrong length should raise
    with pytest.raises(ValueError):
        src.align_with(dst, np.array([1.0, 2.0, 3.0], np.float32))

    # correct length works and dtype respected
    aligned = src.align_with(
        dst, np.array([5.0, 6.0], np.float32), fill=-1.0, dtype=np.float32
    )
